...
public class UserController extends BaseController implements SystemInter {
    ...
    public Result login(@RequestBody Map<String,Object> loginMap){
        ...
        try {
            ...
            if (!Constant.UserLevel.SAASADMIN.equals(user.getLevel())){
                ...
                Object object
                        = companyInter.findCompanyById(user.getCompanyId()).getData();
//                JSON.toJSONString则是将对象转化为Json字符串。JSON.parseObject，是将Json字符串转化为相应的对象
                String json = JSON.toJSONString(object);

                Company company = JSON.parseObject(json, new TypeReference<Company>() {
                });
